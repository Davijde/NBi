...
namespace NBi.Testing.NUnit.Constraint.ResultSetBased.RowPredicate
{
    ...
    public class AllRowsConstraintTest
    {
        ...
        public void ApplyTo_ResultSetResolver_CallToExecuteOnce()
        {
            ...
            Mock.Get(filter).Setup(f => f.Revert()).Returns(filter);
            Mock.Get(filter).Setup(f => f.Execute(It.IsAny<IResultSet>())).Returns(filteredRs);

            var allRowsCtr = new AllRowsConstraint(filter);

            allRowsCtr.ApplyTo(actual);
        ...
        [Test]
        public void ApplyTo_Filter_CallToRevertOnce()
        {
            var actualRs = Mock.Of<IResultSet>();
            Mock.Get(actualRs).Setup(s => s.Rows).Returns(Enumerable.Empty<IResultRow>());
            ...
            allRowsCtr.ApplyTo(actual);
            Mock.Get(filter).Verify(f => f.Revert(), Times.Once());
        }
        ...
        public void ApplyTo_ResultZeroRow_Success()
        {
            ...
            var result = allRowsCtr.ApplyTo(actual);
